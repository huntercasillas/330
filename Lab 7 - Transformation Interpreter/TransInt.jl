...
using Error
using Lexer
export parse, calc, analyze, interp, interpf
...
keyDict = Dict( :+ => +, :- => -, :* => *, :/ => /, :mod => mod, :collatz => collatz,
:if0 => "if0", :with => "with", :lambda => "lambda", :and => "and" )
...
function parse( expr::Array{Any} )
	...
	if key == :+ || key == :and
		if key == :+
			if length( expr ) < 3
				throw( LispError( "Error. Invalid number of arguments." ) )
			else
				body = map( value-> parse( value ), expr[2 : end] )
				return PlusNode( body )
			end
		elseif key == :and
			if length( expr ) < 3
				throw( LispError( "Error. Invalid number of arguments." ) )
	...
	elseif length( expr ) == 2
		...
		if key == :- || key == :collatz
			return BinopNode( keyDict[key], parse( expr[2] ) )
		end
...
function analyze( ast::PlusNode )
    if length( ast.body ) == 2
		return BinopNode( +, analyze( ast.body[1] ), analyze( ast.body[2] ) )
	end

	return BinopNode( +, ast.body[1], analyze( PlusNode( ast.body[2:end] ) ) )
end
...
function analyze( ast::BinopNode )
	...
	return BinopNode( operator, alhs, arhs )
end

function analyze( ast::AndNode )
    if length( ast.body ) == 1
        return If0Node( analyze( ast.body[1] ), NumNode(0), NumNode(1) )
    else
        return If0Node( analyze( ast.body[1] ), NumNode(0), analyze( AndNode( ast.body[2:end] ) ) )
    end
end
...
function analyze( ast::WithNode )
	...
	for ( s, e ) in ast.sym_expr
		push!( sym, s )
	    push!( expr, analyze( e ) )
	end

	fdn = FuncDefNode( sym, analyze( ast.body ) )
	return FuncAppNode( fdn, expr )
end
