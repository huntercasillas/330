...
function type_of_expr(ast::IsZeroNode, env::TypeEnvironment)
    value = type_of_expr(ast.arg, env)
    if value == NumType()
      return BoolType()
    end
    throw(LispError("Error. You must run IsZeroNode with a number."))
end
...
function type_of_expr(ast::VarRefNode, env::TypeEnvironment)
  if env == EmptyTypeEnv()
    val = ast.sym
    throw(LispError("Error. Symbol $val could not be found."))
...
function type_of_expr(ast::NIsEmptyNode, env::TypeEnvironment)
  if type_of_expr(ast.list, env) != NListType()
    throw(LispError("Error. You must run NIsEmptyNode with a list."))
  end
  return BoolType()
end
...
function type_of_expr(ast::NFirstNode, env::TypeEnvironment)
  if type_of_expr(ast.list, env) != NListType()
    throw(LispError("Error. You must run NFirstNode with a list."))
  end
  return NumType()
end
...
function type_of_expr(ast::NConsNode, env::TypeEnvironment)
  if type_of_expr(ast.f) != NumType()
    throw(LispError("Error. You must run NConsNode with a list of numbers."))
  elseif type_of_expr(ast.r) != NListType()
    throw(LispError("Error. You must run NConsNode with a list."))
  end
  return NListType()
end
...
function type_of_expr(ast::IfBNode, env::TypeEnvironment)
  if type_of_expr(ast.cond, env) != BoolType()
    throw(LispError("Error. You must run IfbNode with a boolean."))
  end
  tbType = type_of_expr(ast.nzerobranch, env)
  fbType = type_of_expr(ast.zerobranch, env)
  if !same_type(tbType, fbType)
    throw(LispError("Error. IfBNode return types are not equal."))
  end
  return tbType
end

function type_of_expr(ast::FuncDefNode, env::TypeEnvironment)
  newEnvironment = ExtendedTypeEnv(ast.formal_valueeter, ast.formal_type, env)
...
function type_of_expr(ast::FuncAppNode, env::TypeEnvironment)
  ...
  if arg_type != func_type.arg_type
    throw(LispError("Error. FuncAppNode operator and parameter types are not equal."))
  end
